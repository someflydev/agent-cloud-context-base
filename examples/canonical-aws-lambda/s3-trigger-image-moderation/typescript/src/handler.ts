export type Clients = {
  claimObjectVersion: (dedupeKey: string, object: unknown) => Promise<boolean>;
  detectLabels: (bucket: string, key: string) => Promise<unknown[]>;
  detectModerationLabels: (bucket: string, key: string) => Promise<unknown[]>;
  recordDecision: (dedupeKey: string, decision: string, labels: unknown[]) => Promise<void>;
  publishFlagged: (detail: unknown) => Promise<void>;
};
