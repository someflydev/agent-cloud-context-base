# Blob Trigger Receipt OCR - TypeScript

TypeScript Azure Function for Blob Storage create events. The handler sends
...
- Secret path: `/accb/dev/azure-functions/blob-receipt-ocr/` and
  `/accb/test/azure-functions/blob-receipt-ocr/` in Key Vault references.
