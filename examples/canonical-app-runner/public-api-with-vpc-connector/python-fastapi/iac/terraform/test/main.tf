...
variable "vpc_id" {
  type    = string
  default = "vpc-replace-me-test"
}
variable "subnet_ids" {
  type    = list(string)
  default = ["subnet-replace-me-test-a", "subnet-replace-me-test-b"]
}
...
resource "awscc_secretsmanager_secret" "db_test" {
...
resource "aws_apprunner_service" "service" {
  ...
  source_configuration {
    ...
    image_repository {
      image_identifier      = "${aws_ecr_repository.service.repository_url}:test"
      image_repository_type = "ECR"
      image_configuration {
        port = "8080"
        runtime_environment_variables = {
          DB_HOST        = aws_rds_cluster.aurora.endpoint
          DB_SECRET_PATH = awscc_secretsmanager_secret.db_test.name
        }
      }
    }
  }
