# Container Apps public API, private worker, and jobs

Python FastAPI seed for `accb` Azure Container Apps generation.
...
- State: Terraform `dev` and `test` backends use distinct keys; Pulumi stacks
  are `dev` and `test`.
- Env-var prefix: `ACCB_ACA_PUBLIC_WORKER_DEV_` and
  `ACCB_ACA_PUBLIC_WORKER_TEST_`.
- Secret path: `/accb/dev/aca/public-worker/api-key` and
  `/accb/test/aca/public-worker/api-key`.
- Resource naming: `accb-${environment}-aca-public-worker-*`.
...
```bash
bash examples/canonical-container-apps/public-api-private-worker-jobs/python-fastapi/tests/verify.sh
```
