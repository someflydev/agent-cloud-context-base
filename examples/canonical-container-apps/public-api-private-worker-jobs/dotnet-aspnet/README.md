# Container Apps public API, private worker, and jobs (.NET)

.NET ASP.NET seed for `accb` Azure Container Apps generation.
...
- State: Terraform `dev` and `test` backends use distinct keys; Pulumi stacks
  are `dev` and `test`.
- Env-var prefix: `ACCB_ACA_PUBLIC_WORKER_DOTNET_DEV_` and
  `ACCB_ACA_PUBLIC_WORKER_DOTNET_TEST_`.
- Secret path: `/accb/dev/aca/public-worker-dotnet/api-key` and
  `/accb/test/aca/public-worker-dotnet/api-key`.
- Resource naming: `accb-${environment}-aca-public-worker-dotnet-*`.
...
```bash
bash examples/canonical-container-apps/public-api-private-worker-jobs/dotnet-aspnet/tests/verify.sh
```
