...
variable "image" {
  type    = string
  default = "gcr.io/example/accb-test-cloudrun-public-worker-go:latest"
}
...
locals {
  name_prefix = "accb-${var.environment}-cloudrun-public-worker-go"
  labels      = { managed_by = "accb", environment = var.environment, family = "canonical-cloud-run" }
}

resource "google_service_account" "service" {
  account_id   = "${local.name_prefix}-svc"
  display_name = "accb ${var.environment} Cloud Run public worker Go"
}
