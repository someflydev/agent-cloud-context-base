...
variable "image" {
  type    = string
  default = "gcr.io/example/accb-test-cloudrun-public-worker-ts:latest"
}
...
locals {
  name_prefix = "accb-${var.environment}-cloudrun-public-worker-ts"
  labels      = { managed_by = "accb", environment = var.environment, family = "canonical-cloud-run" }
}

resource "google_service_account" "service" {
  account_id   = "${local.name_prefix}-svc"
  display_name = "accb ${var.environment} Cloud Run public worker TypeScript"
}
